...
if (player != leader player) exitWith {hint "You cannot control AI if you are not the squad leader"};
...
waitUntil {sleep 1; hint format ["Time to return control to AI: %1", _tiempo]; _tiempo = _tiempo - 1; (_tiempo == -1) or (isPlayer (leader player))};
