...
If (_AttackPos select 0 != 0 && _AttackPos select 1 != 0) then
{
	...
	_dir = _AttackPos getdir getposATL _npc;
}
else
{
	_dir = getposATL _npc getDir (_grp getvariable "UPSMON_Origin") select 2;
};
...
[_grp,_retreatPos,_wptype,_wpformation,_speedmode,_behaviour,_CombatMode,1] spawn UPSMON_DocreateWP;
