...
_edificios = _edificios select {((_x buildingPos -1) isEqualTo []) and !((typeof _x) in UPSMON_Bld_remove) and (_x inArea _marcador) };
...
while { true } do {
	...
	if !(_objetivos isEqualTo []) then {
		...
		_allNearFriends = allUnits select {
			(_x distance _lider < (distanciaSPWN / 2)) and (side _x in _friendlies) and ([_x] call A3A_fnc_canFight) }; {
			_unit = _x; {
		...
		_soldados = ((units _grupo) select {
			[_x] call A3A_fnc_canFight }) - [_grupo getVariable ["mortero", objNull]];
		_numSoldados = count _soldados;
		if !(isNull _aire) then {
			if ({
					(_x call A3A_fnc_typeOfSoldier == "AAMan") or (_x call A3A_fnc_typeOfSoldier == "StaticGunner") } count _allNearFriends == 0) then {
				if (_lado != buenos) then {
					[
						[getPosASL _lider, _lado, "Air", false], "A3A_fnc_patrolCA"
		...
		if !(isNull _tanques) then {
			if ({ _x call A3A_fnc_typeOfSoldier == "ATMan" } count _allNearFriends == 0) then {
				_mortero = _grupo getVariable ["morteros", objNull];
				if (!(isNull _mortero) and ([_mortero] call A3A_fnc_canFight)) then {
					if ({ if (_x distance _tanques < 100) exitWith { 1 } } count _allNearFriends == 0) then {
						[_mortero, getPosASL _tanques, 4] spawn A3A_fnc_mortarSupport };
				}
				else {
					if (_lado != buenos) then {
						[
							[getPosASL _lider, _lado, "Tank", false], "A3A_fnc_patrolCA"
						] remoteExec ["A3A_fnc_scheduler", 2] };
				};
			};
		...
		if (_numObjetivos > 2 * _numNearFriends) then {
			if !(isNull _cercano) then {
				if (_lado != buenos) then {
					[
						[getPosASL _lider, _lado, "Normal", false], "A3A_fnc_patrolCA"
					] remoteExec ["A3A_fnc_scheduler", 2] };
				_mortero = _grupo getVariable ["morteros", objNull];
				if (!(isNull _mortero) and ([_mortero] call A3A_fnc_canFight)) then {
			...
			if (_cercano distance _lider < 50) then {
				...
				if (_cercano distance _lider > 150) then {
					...
					if !(isNull _cercano) then {
						...
						forEach ((_grupo getVariable ["baseOfFire", []]) select {
							([_x] call A3A_fnc_canFight) and ((_x getVariable ["typeOfSoldier", ""] == "MGMan") or (_x getVariable ["typeOfSoldier", ""] == "StaticGunner")) });
						_mortero = _grupo getVariable ["morteros", objNull];
						if (!(isNull _mortero) and ([_mortero] call A3A_fnc_canFight)) then {
							if ({ if (_x distance _cercano < 100) exitWith { 1 } } count _allNearFriends == 0) then {
								[_mortero, getPosASL _cercano, 1] spawn A3A_fnc_mortarSupport };
		...
		if (_tarea == "AssaultClose") then {
			if (_cercano distance _lider > 150) then {
				...
				if (_cercano distance _lider > 50) then {
					...
					if !(isNull _cercano) then {
						_flankers = (_grupo getVariable ["flankers", []]) select {
							([_x] call A3A_fnc_canFight) and!(_x getVariable ["maniobrando", false]) };
						if (count _flankers != 0) then {
							...
							if ([getPosASL _cercano] call A3A_fnc_isBuildingPosition) then {
								...
								if !(_building getVariable ["asaltado", false]) then {
									{
										if ((_x call A3A_fnc_typeOfSoldier == "Engineer") and { _x != leader _x } and {!(_x getVariable ["maniobrando", true]) } and { _x distance _cercano < 50 } and {
												[_x] call A3A_fnc_canFight }) exitWith { _ingeniero = _x };
									}
									...
									if !(isNull _ingeniero) then {
										...
										[
											[_flankers, _cercano] call BIS_fnc_nearestPosition, _cercano, _building
										] spawn A3A_fnc_assaultBuilding;
									};
								};
							}
							else {
		...
		if (_tarea == "Hide") then {
			if ((isNull _tanques) and { isNull _aire } and { _numObjetivos <= 2 * _numNearFriends }) then {
				...
				_movable = (_grupo getVariable ["movable", []]) select {
					[_x] call A3A_fnc_canFight and!(_x getVariable ["maniobrando", false]) };
		...
		if (side _grupo == buenos) then {
			if (time >= _grupo getVariable ["autoRearm", time]) then {
				_grupo setVariable ["autoRearm", time + 120]; {
					[_x] spawn A3A_fnc_autoRearm; sleep 1 } forEach ((_grupo getVariable ["movable", []]) select {
					[_x] call A3A_fnc_canFight and!(_x getVariable ["maniobrando", false]) });
			};
		};
	};
