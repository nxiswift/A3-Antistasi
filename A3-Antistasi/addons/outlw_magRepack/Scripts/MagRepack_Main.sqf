...
outlw_MR_createDialog =
{
	...
	((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 1001) ctrlSetText ("Перепаковка [" + outlw_MR_version + "]");
	...
	((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 22170) lbAdd "Все типы маг.";
	...
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9002) ctrlSetStructuredText parseText "Ошибки: <t align='right'>ВКЛ</t>";
	}
	else
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9002) ctrlSetStructuredText parseText "Ошибки: <t align='right'>ВЫКЛ</t>";
	};

	if (outlw_MR_doHideFull) then
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9004) ctrlSetStructuredText parseText "Показ все: <t align='right'>ВЫКЛ</t>";
	}
	else
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9004) ctrlSetStructuredText parseText "Показ все: <t align='right'>ВКЛ</t>";
	};

	((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9003) ctrlSetStructuredText parseText "Клавиши";
	((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9005) ctrlSetStructuredText parseText "О нас";
...
outlw_MR_onDialogDestroy =
{
	...
	if (outlw_MR_debugMode) then
	{
		...
		for "_n" from 0 to ((count _sTCA) - 1) do
		{
			...
			_toAdd = ((getText (configFile >> "cfgMagazines" >> _snTCA select 0 >> "DisplayName")) + "<br/>[" + str(_snTCA select 1) + "] >> [0]<br/>Патр: <t color='#FC1010'>-" + str(_snTCA select 2) + "</t>");
			...
			for "_a" from 0 to ((count _eTCA) - 1) do
			{
				...
				if ((_snTCA select 0) == (_enTCA select 0)) then
				{
					...
					switch (true) do
					{
						case (_dif > 0): {_toAdd = (_toAdd + "Патр: <t color='#15E612'>+" + str(_dif) + "</t>")};
						case (_dif < 0): {_toAdd = (_toAdd + "Патр: <t color='#FC1010'>" + str(_dif) + "</t>")};
						default {_toAdd = (_toAdd + "Патр: No Change")};
					};
				};
			};
		...
		for "_n" from 0 to ((count _eTCA) - 1) do
		{
			...
			_toAdd = ((getText (configFile >> "cfgMagazines" >> _enTCA select 0 >> "DisplayName")) + "<br/>[0] >> [" + str(_enTCA select 1) + "]<br/>Патр: <t color='#15E612'>+" + str(_enTCA select 2) + "</t><br/><br/>");
		...
		hint parseText ("<br/><t size='1.05'>Mag Repack Debug</t><t size='0.95'><br/><br/>Все маг<br/>[" + str((outlw_MR_startingInfo select 0) select 0) + "] >> [" + str((_endingInfo select 0) select 0) + "]<br/>Патр: " + _difStr + "<br/><br/>" + _output + "</t>");
	};
