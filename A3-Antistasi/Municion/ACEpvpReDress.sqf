...
if (hayACEMedical) then {
	...
	if !([player] call A3A_fnc_isMedic) then {
		...
		{
			_item = _x select 0;
			for "_i" from 1 to (_x select 1) do { player addItemToUniform _item; };
		...
		{
			_item = _x select 0;
			for "_i" from 1 to (_x select 1) do { player addItemToBackpack _item; };
...
if ((player getUnitTrait "explosiveSpecialist") or (player getUnitTrait "engineer")) then {
	...
	{
		_item = _x select 0;
		for "_i" from 1 to (_x select 1) do { player addItemToVest _item; };
